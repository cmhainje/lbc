...
class _Encoder(eqx.Module):
    ...
    def __init__(
        ...
        self.layers = [
            ...
            jax.nn.relu,
            Conv2d(chan, chan, key=keys[2], **conv_kw),
            jax.nn.relu,
            Conv2d(chan, chan, key=keys[3], **conv_kw),
            AvgPool2d(**pool_kw),  # 64x64 -> 32x32
            jax.nn.relu,
            Conv2d(chan, 2 * chan, key=keys[4], **conv_kw),
            jax.nn.relu,
            Conv2d(2 * chan, 2 * chan, key=keys[5], **conv_kw),
            AvgPool2d(**pool_kw),  # 32x32 -> 16x16
            jax.nn.relu,
            jax.numpy.ravel,
            Linear(2 * chan * 16 * 16, latent_dim, key=keys[6]),
        ]
...
class _Decoder(eqx.Module):
    ...
    def __init__(
        ...
        self.layers = [
            ConvTranspose2d(2 * chan, 2 * chan, key=keys[1], **convT_kw),  # 16x16 -> 32x32
            jax.nn.relu,
            Conv2d(2 * chan, chan, key=keys[2], **conv_kw),
            jax.nn.relu,
            Conv2d(chan, chan, key=keys[3], **conv_kw),
            jax.nn.relu,
            ConvTranspose2d(chan, chan, key=keys[4], **convT_kw),  # 32x32 -> 64x64
            jax.nn.relu,
            Conv2d(chan, chan, key=keys[5], **conv_kw),
            jax.nn.relu,
            Conv2d(chan, chan, key=keys[6], **conv_kw),
            jax.nn.relu,
            ConvTranspose2d(chan, chan, key=keys[7], **convT_kw),  # 64x64 -> 128x128
            jax.nn.relu,
            Conv2d(chan, chan, key=keys[8], **conv_kw),
            jax.nn.relu,
            Conv2d(chan, 1, key=keys[9], **conv_kw),
